...
def nasdaq_lstm(cfg):
    if cfg.base.mode=='train':
        ...
        # 다시.. 마치 set_index되게끔..
        train = pd.read_csv(opj(cfg.base.data_dir, 'train_nasdaq.csv'), index_col=0)
        val = pd.read_csv(opj(cfg.base.data_dir, 'val_nasdaq.csv'), index_col=0)
        test = pd.read_csv(opj(cfg.base.data_dir, 'test_nasdaq.csv'), index_col=0)
        ###################################################################
        train = train.fillna(method='bfill')
        val = val.fillna(method='bfill')
        test = test.fillna(method='bfill')
